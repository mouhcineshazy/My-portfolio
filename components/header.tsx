    ...
    <header className="z-[999] relative">
      <motion.div
        ...
        backdrop-blur-[0.5rem] sm:top-6 sm:h-[3.25rem] sm:rounded-full dark:bg-gray-950 dark:border-black/40 dark:bg-opacity-75 ${
          locale === 'en' ? 'sm:w-[36rem]' : 'sm:w-[38rem]'
        }`,
        )}
      ...
      <nav className="flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0 ">
        <ul
          className={clsx(
            `flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap ${
              locale === 'en' && 'sm:gap-5'
            }`,
          )}
        >
